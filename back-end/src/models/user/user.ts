...
export class User extends SEntity implements Searchable<User>, hasPermission {
	static DB_TABLE = { PRIM: CONFIG.DB.TABLE_PREFIX + "user", RELATIONAL: {} };
	public name: string;
	email: string;
	birthDate: Date;
	...
	private async hashPassword(): Promise<boolean> {
		let saltRounds = 8;
	 ...
	 */
	public login(email: string, password: string): boolean { return false; }
	 ...
	 */
	public logout(): boolean { return false; }

	public sendNotification(subject?: any, Notification?: any): any {
		return null;
	}

	public search(query: string): User[] { return null; }

	public hasPermission(): Permission { return null; }
}