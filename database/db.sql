...
CREATE TABLE IF NOT EXISTS `my_uni`.`course_offering_enrollment` (
  ...
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `semester_enrollment_id` INT NOT NULL,
  `course_offering_id` INT NOT NULL,
  `group_id` INT NOT NULL,
  `section_id` INT NULL,
  ...
  INDEX `fk_enrollment_has_group_enrollment1_idx` (`semester_enrollment_id` ASC),
  PRIMARY KEY (`id`),
  ...
  UNIQUE INDEX `enrollment_id_and_course_UNIQUE` (`semester_enrollment_id` ASC, `course_offering_id` ASC),
  CONSTRAINT `fk_enrollment_has_group_enrollment1`
    FOREIGN KEY (`semester_enrollment_id`)
    REFERENCES `my_uni`.`semester_enrollment` (`id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
